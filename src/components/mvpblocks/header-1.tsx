...
export default function Header1() {
  ...
  style={{
    backgroundColor: isScrolled
      ? 'rgba(6, 78, 59, 0.95)'
      : 'transparent',
    boxShadow: isScrolled ? '0 8px 32px rgba(5, 150, 105, 0.2)' : 'none',
  }}
>
        ...
        <div className="flex h-16 items-center justify-between lg:h-20">
          ...
          >
            ...
            >
              ...
              <span className="bg-gradient-to-r from-green-300 to-green-600 bg-clip-text text-xl font-bold text-transparent">
                Krishi_Ai
              </span>
            </Link>
          </motion.div>
            ...
            {navItems.map((item) => (
              ...
              >
                <Link
                  prefetch={false}
                  href={item.href}
                  className="text-green-400 flex items-center space-x-1 font-medium transition-colors duration-200 hover:text-green-700"
                >
                  <span>{item.name}</span>
                  {item.hasDropdown && (
          ...
          <div className="hidden items-center space-x-4 lg:flex">
            <Link
              prefetch={false}
              href="/login"
              className="text-green-500 font-medium transition-colors duration-200 hover:text-green-700"
            >
              Sign In
            </Link>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Link
                prefetch={false}
                href="/signup"
                className="inline-flex items-center space-x-2 rounded-full bg-gradient-to-br from-green-500 to-green-700 px-6 py-2.5 font-medium text-white transition-all duration-200 hover:shadow-lg"
              >
